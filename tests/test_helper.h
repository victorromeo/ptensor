...
#define PTENSOR_TEST_CONFIGURE() std::vector<Case> cases(); \
    Serial pc(USBTX, USBRX, 115200); \
+SDBlockDevice bd(MBED_CONF_APP_SD_MOSI, MBED_CONF_APP_SD_MISO, MBED_CONF_APP_SD_CLK, MBED_CONF_APP_SD_CS); \
+FATFileSystem fs("fs");

#define PTENSOR_TEST(x, y, message) cases.push_back(Case( message, test_ ## x ## _ ## y ));

#define PTENSOR_TEST_RUN() Specification specification(greentea_setup, cases.data); \
    int main(){ \
...
#define PTENSOR_TEST_CONFIGURE() /* pass */

#define PTENSOR_TEST(x, y, message) GTEST_TEST(x, y){ test_ ## x ## _ ## y(); }

// Google does this better than I can
#define PTENSOR_TEST_RUN() int main(int argc, char** argv) { \
