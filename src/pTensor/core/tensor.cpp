...
// Force everything to be on the ptensor allocator
...
void print(const Tensor& t) {
  ...
  if (t_shape.num_dims() > 2) {
    pTensor_printf("printing > 2D tensors not supported\n");
    return;
  }
  pTensor_printf("[\n");
  for (int j = 0; j < t_shape[1]; j++) {
    ...
    for (int i = 0; i < t_shape[0]; i++) {
      switch (t->get_type()) {
        case u8:
          pTensor_printf("%hhu", static_cast<uint8_t>(t(j, i)));
          break;
        case i8:
          pTensor_printf("%hhd", static_cast<int8_t>(t(j, i)));
          break;
        case u16:
          pTensor_printf("%hu", static_cast<uint16_t>(t(j, i)));
          break;
        case i16:
          pTensor_printf("%hd", static_cast<int16_t>(t(j, i)));
          break;
        case u32:
          pTensor_printf("%u", static_cast<uint32_t>(t(j, i)));
          break;
        case i32:
          pTensor_printf("%d", static_cast<int32_t>(t(j, i)));
          break;
        case flt:
          pTensor_printf("%f", static_cast<float>(t(j, i)));
          break;
        default:
          pTensor_printf("Unknown data type");
          return;
      }
      if (i != (t_shape[0] - 1))
        pTensor_printf(", ");
...
SimpleNamedTensor::SimpleNamedTensor(const pTensor::string& name,
                                     Tensor& tensor)
