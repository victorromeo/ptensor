...
#define pTensor_printf(...) printf(__VA_ARGS__)
#else
#define pTensor_printf(...) /*DUMMY*/
#endif
...
#define DEBUG(MSG, ...)         \
  {                             \
    pTensor_printf(MSG, ##__VA_ARGS__); \
    fflush(stdout);             \
  }
...
void ptensor_exit(void);

#define ERR_EXIT(MSG, ...)                                      \
  {                                                             \
    pTensor_printf("[Error] %s:%d @%s ", __FILE__, __LINE__, __func__); \
    pTensor_printf(MSG, ##__VA_ARGS__);                                 \
    fflush(stdout);                                             \
    ptensor_exit();                                             \
  }
