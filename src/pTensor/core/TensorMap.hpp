#ifndef PTENSOR_TENSOR_MAP
#define PTENSOR_TENSOR_MAP
...
class TensorMapInterface {
 public:
  virtual SimpleNamedTensor& operator[](const pTensor::string& name) = 0;
  virtual const SimpleNamedTensor& operator[](
      const pTensor::string& name) const = 0;
  virtual bool has(const pTensor::string& name) const = 0;
  /*
...
class FixedTensorMap : public TensorMapInterface {
 public:
  ...
  FixedTensorMap(initializer_list<SimpleNamedTensor> l) {
    if (size != l.size()) {
      // TODO THROW ERROR
      pTensor_printf("[Warning] Element number mismatch in TensorMap construction\n");
    }
    int i = 0;
  ...
  */
  ...
  virtual const SimpleNamedTensor& operator[](
      const pTensor::string& name) const override {
    if (!has(name)){
      return TensorMapInterface::not_found;
    }
    return _map[name.get_value()];
  ...
  virtual bool has(const pTensor::string& name) const override {
